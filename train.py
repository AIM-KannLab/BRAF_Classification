...
def train(root_dir, out_dir, log_dir, model_dir, model, cnn_model, train_gen, 
          ...
          freeze_layer, trained_weights, transfer_learning): 
    ...
    if transfer_learning:
        cnn_config = cnn_model + "_"
    else: 
        cnn_config = cnn_model + "_scratch"

    check_point = tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(model_dir, task + '_' + "external_finetune_v600e_"  + cnn_config + '_{epoch:02d}_{val_auc:.2f}.h5'),
        monitor='val_auc',  
        save_best_only=True,
    ...
    model.save(model_dir + '/' + str(cnn_model) + "v600e" + '_final_model')
