...
def img_data(save_dir, df, fn_arr_1ch, fn_arr_3ch, fn_df, channel, save_nii, nii_dir):
    ...
    print(df)
    for img_path, pat_id, zmin, zmax in zip(df['image path'], df['bch mrn'], df['bottom z index'], df['top z index']):       # change img_path to seg_path here to create a dataset array of image 
        count += 1
        print(count, pat_id)
        img = resize_3d(
            img_dir=img_path, 
...
def get_img_dataset(proj_dir, channel, save_nii):
    ...
    save_dir = proj_dir + '/2d_data'
    nii_dir = proj_dir + '/nii_data'

    # get data sets
    df_tx = pd.read_csv(os.environ.get("PWD") + "/zmin_zmax.csv")
    df = pd.read_csv(os.environ.get("PWD") + "/zmin_zmax.csv")
    ...
    dfs = [df]
    fns_arr_1ch = ['tr_arr_1ch.npy']
    fns_arr_3ch = ['img_tr_arr_3ch.npy']
    fns_df = ['tr_img_df.csv']
...
if __name__ == '__main__':
    ...
    proj_dir = os.environ.get("PWD") + '/BRAF'
    channel = 3
